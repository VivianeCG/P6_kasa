    ...
    return(
            <section className='collapse_element '>
                <div className='collapse_header'>
                    ...
                    <button  className={`collapse_button ${isOpen ? 'open' : ''}`} onClick={toggleCollapse}>
                    <svg width="33" height="32" viewBox="0 0 33 32" fill="none" xmlns="http://www.w3.org/2000/svg"  className={`collapse_arrow ${isOpen ? 'open' : ''}`}>
                ...
                <article className={`collapse_content ${isOpen ? 'open' : ''}`}>{children}</article>
            </section>
